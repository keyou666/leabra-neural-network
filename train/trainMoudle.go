...
/*
Because we want to study the pre & post test effect,
we have 3 main process to implement at lease（RP,TRAIN,TEST).
Maybe finally, preTraining and RS processes may be added.
*/

// Sim
/*
adhesive for each module
*/
type Sim2 struct {
	Net     *leabra.Network
	Hp      *model.HipParams
...
/*
all the process have the 3 main function

Train(process entry) --> Trial(traverse every sample) --> AlphaCyc(specific process)
*/

func (s *Sim2) PreTrainInit(BaseInfo string, dataName string) {
...
func (s *Sim2) PreTrainInitTrail(BaseInfo string) bool {
	s.TrainEnv.Step()
...
func (s *Sim2) TrainRP(BaseInfo string, dataName string) {
	s.TrainEnv.Table = etable.NewIdxView(s.DataMap[dataName])
	s.TrainEnv.Init(0)
...
func (s *Sim2) RetrievalPracticeTrial(BaseInfo string) bool {
	s.TrainEnv.Step()
...
func (s *Sim2) AlphaCycRP(trailBaseInfo *TrailBaseInfo, train bool) {
	if train {
		s.Net.WtFmDWt()
	}
	dg := s.Net.LayerByName("DG").(leabra.LeabraLayer).AsLeabra()
...
func (s *Sim2) Train(BaseInfo string, dataName string) {
	s.TrainEnv.Table = etable.NewIdxView(s.DataMap[dataName])
	s.TrainEnv.Init(0)
	s.TrainEnv.Trial.Cur = -1
	for {
...
func (s *Sim2) TrainTrail(BaseInfo string) bool {
	s.TrainEnv.Step()
	_, _, chg := s.TrainEnv.Counter(env.Epoch)
	if chg {
		EpochStat(s.TrnTrlLog, s.TrnEpcLog, BaseInfo, int64(s.TrainEnv.Run.Cur), int64(s.TrainEnv.Epoch.Cur)-1, fmt.Sprint(s.Pa.ListSize), true)
...
func (s *Sim2) AlphaCyc(trailBaseInfo *TrailBaseInfo, train bool) {
	dg := s.Net.LayerByName("DG").(leabra.LeabraLayer).AsLeabra()
	ca1 := s.Net.LayerByName("CA1").(leabra.LeabraLayer).AsLeabra()
	ca3 := s.Net.LayerByName("CA3").(leabra.LeabraLayer).AsLeabra()
	input := s.Net.LayerByName("Input").(leabra.LeabraLayer).AsLeabra()
...
func (s *Sim2) Test(BaseInfo string, dataName string) {
...
func (s *Sim2) TestTrial(BaseInfo string) bool {
	s.TestEnv.Step()
...
func (s *Sim2) applyInputs(train bool) {
